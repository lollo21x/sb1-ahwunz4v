...
import { getPrimaryModels, getDefaultModel } from '../services/models';
...
export const useModel = (onModelChange?: () => void) => {
  const [selectedModel, setSelectedModel] = useState<ModelType>(() => {
    try {
      ...
      if (saved && getPrimaryModels().some(m => m.id === saved)) {
  ...
  const currentModel: ModelInfo = getPrimaryModels().find(m => m.id === selectedModel) || getPrimaryModels()[0]; // Default to GPT-OSS
